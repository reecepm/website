...
export default function MonospaceTerminalNavigator({ initialSlug, about }: Props) {
  ...
  return (
    <div className="fixed left-0 right-0 bottom-0 z-[100] font-mono pointer-events-none [&>*]:pointer-events-auto">
      ...
      <div className="flex flex-row items-center h-10 px-8 gap-4 bg-[var(--mt-bg)] border-t border-[var(--mt-border)] text-[var(--mt-text)] text-xs">
        <span className="text-[var(--mt-accent)] font-medium text-[13px]">$</span>
        <span className="text-[var(--mt-muted)]">~/themes/</span>
        ...
        <span className="text-[var(--mt-border)]">·</span>
        <span className="text-[var(--mt-muted)]">{author}</span>
        <span className="text-[var(--mt-border)]">·</span>
        <span className="text-[var(--mt-text-secondary)] flex-1 whitespace-nowrap overflow-hidden text-ellipsis min-w-0 opacity-70">
          {description}
        </span>
        ...
        <div className="relative inline-flex" ref={infoWrapperRef}>
          ...
          {infoOpen && (
            <div
              className="absolute bottom-[calc(100%+16px)] right-0 w-[360px] bg-[var(--mt-bg)] border border-[var(--mt-accent)] px-5 pt-[18px] pb-3.5 flex flex-col gap-2.5 z-[101]"
              role="dialog"
        ...
        <span className="inline-block w-2 h-3.5 bg-[var(--mt-accent)] ml-1 shrink-0 animate-[mt-blink_1.2s_step-end_infinite]" />
      </div>
    </div>
  );
}
